...
; DONE       chirp/interrupt the sound at 13 cm so you know when you are in range of touching the object 5/11/2016      
;******************************************************************* 
        ...
        __config  _INTRC_OSC_NOCLKOUT & _LVP_OFF & _WDT_OFF & _PWRTE_ON & _BODEN_ON  & _CP_OFF & _MCLRE_OFF
...
chirp:
		; entering this with TMR0 in w
		sublw	.32				; subtract w from 32
		btfsc	STATUS,C		; if w less than .32 carry will be set (C=1 if result positive, see SUBLW inst)
		goto	chirpnear
		return
chirpnear:
		bcf		INTCON,GIE	; stop sound by turning off interrupts which toggle the sound pin
		call	Delay30ms
		bsf		INTCON,GIE	; re-enable sound after 30ms
		call	Delay60ms	; although this slows response time it makes the chirping eaier to hear and the freq changes easier to hear in the chirping
		return
